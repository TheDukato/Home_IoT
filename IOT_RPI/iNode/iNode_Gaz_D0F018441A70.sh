	...
	DTYPE=0
		...
		if [[ "$DTYPE" -eq "130" ]]; then # iNode Energy Meter
		       ...
		       #echo MINUTE POWER $DEC
                       CALC=`echo $DEC $CONST | awk '{ kw=(($1*60.0)/$2); printf"%0.2f\n",  kw  }'`
		       ...
		       if [ "$MAC" = "D0F018441A70" ]; then
			       echo Mamy dopasowanie !!!
			       echo MAC: $MAC POWER: $CALC kW TOTAL: $CALCIM kWh | awk '{ print strftime("%Y-%m-%d %H:%M:%S"), $0; fflush(); }' >> sensor_main_gas.txt
			       exit 1
		       fi
		       #actual_data=$(date +"%Y-%m-%d_%H:%M:%S")
		       #echo actual_data $actual_data
	 	       #echo sqlite3 data "INSERT INTO meterSensor VALUES ('iNode_"$MAC"','"$actual_data"',"$CALCIM",'kWh');"
		       #if [[ "$MAC" -eq "D0F01843D74F" ]]; then
                       #sqlite3 data "INSERT INTO meterSensor VALUES ('iNode_"$MAC"','"$actual_data"',"$CALCIM",'kWh');"
		       #fi
